<?php
	...
	if (! isset ( $_SERVER ['__translate__'] )) {
		...
		foreach ( App_Translate::getAll ( $pdo ) as $record ) {
			...
			$_SERVER ['__translate__'] [$record ['atr_table']] [$record ['atl_lang']] [$record ['atr_data_field']] [trim ( $record ['atl_data_id'] )] = trim ( $record ['atl_data_lib'] );
		}
	}
