...
$_fac_id = trim ( $_GET ['paramfacture'] );
...
$var = utf8_decode ( 
		...
		$params ['par_complement'] . _STLG . $params ['par_codepostal'] . ' ' . $params ['par_ville'] . ' - ' . $params ['par_pays'] . _STLG . $params ['par_email'] . _STLG . $params ['par_portable'] . ' - ' . $params ['par_telfixe'] . _STLG . _STLG . 'SIRET: ' . $params ['par_siret'] . ' / APE: ' . $params ['par_codenaf'] . _STLG . 'IBAN: ' . $params ['par_iban'] . _STLG . 'TVA Intracom: ' . $params ['par_TvaIntracom'] );
$pdf->MultiCell ( 90, _HTLG2, utf8_decode ( $var ), 0, 'L' );
$pdf->ln ( 2 );

$pdf->SetXY ( - 100, 30 );
$pdf->SetFont ( 'Arial', 'BU', 16 );
$pdf->Cell ( 80, _HTLG1, utf8_decode ( 'FACTURE' ), 0, 0, 'C' );

$pdf->SetXY ( - 100, 50 );
$pdf->SetFont ( 'Arial', '', 12 );
$var = utf8_decode ( $client ['cli_civ_code'] . ' ' . $client ['cli_prenom'] . ' ' . $client ['cli_nom'] . _STLG . $client ['cli_email'] . _STLG . _STLG . $client ['cli_rue'] . _STLG . $client ['cli_complement'] . _STLG . $client ['cli_codepostal'] . ' ' . $client ['cli_ville'] . _STLG . $client ['cli_pays'] );
$pdf->MultiCell ( 90, _HTLG1, utf8_decode ( $var ), 1, 'C' );

$pdf->SetY ( 100 );
$pdf->SetFont ( 'Arial', '', 14 );
$var = utf8_decode ( 'Facture n.' . substr ( $entete ['fac_date'], 0, 4 ) . '-' . $_fac_numero . '  du  ' . date_format ( date_create ( $entete ['fac_date'] ), 'd/m/Y' ) );
$pdf->Cell ( 80, _HTLG1, utf8_decode ( $var ), 0, 1 );

$pdf->SetX ( - 100 );
$pdf->SetFont ( 'Arial', '', 10 );
$pdf->Cell ( 90, _HTLG1, 'facture en euros', 0, 1, 'R' );
...
foreach ( $reglts as $ligne ) {
	...
	$data [] = utf8_decode ( '[C]' . date_format ( date_create ( $ligne ['reg_date'] ), 'd/m/Y' ) );
